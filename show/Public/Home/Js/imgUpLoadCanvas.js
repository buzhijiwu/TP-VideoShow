...
(function () {
        ...
        reader.onload = function (ev) {
            ...
            img.onload = function () {
                ...
                if(imgWidth > 400 && imgHeight >= 400){
                    ...
                    common.alertAuto(false,validate.mistake48);
                    form.reset();
                    context.clearRect(0,0,400,400);
    ...
    function move () {
        return function (ev) {
            canvasTop = canvas1.offsetTop;
            canvasLeft = canvas1.offsetLeft - 46;

            mouseLeft = ev.clientX;
            mouseTop = ev.clientY;

            inMouseLeft = parseInt(mouseLeft - canvasLeft - defaultLeft);
            inMouseTop = parseInt(mouseTop - canvasTop - defaultTop);

            if(inMouseLeft >= lastLeft  && inMouseLeft <= parseInt(lastLeft + afterScaleWidth - 5)  && inMouseTop >= lastTop  && inMouseTop <= parseInt(lastTop + afterScaleWidth - 5)  ){
    ...
    submitBtn.onclick = function (e) {
        ...
        oReq.onreadystatechange = function () {
            if (oReq.readyState==4 && oReq.status==200) {
                if(JSON.parse(oReq.responseText).status == 1){
                    common.alertAuto(true,JSON.parse(oReq.responseText).msg);
                }else{
                    common.alertAuto(false,JSON.parse(oReq.responseText).msg);
                }
            }
        };

        oReq.open("POST", "/Usercenter/updateBigheadpic", true);
